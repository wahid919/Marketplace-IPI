...
class ActionMidtransApiRequestor
{
    ...
    public static function remoteCall($url, $server_key, $data_hash, $method)
    {
        ...
        if (ActionMidtransConfig::$appendNotifUrl) ActionMidtransConfig::$curlOptions[CURLOPT_HTTPHEADER][] = 'X-Append-Notification: ' . ActionMidtransConfig::$appendNotifUrl;
        // Set override notification to header
        if (ActionMidtransConfig::$overrideNotifUrl) ActionMidtransConfig::$curlOptions[CURLOPT_HTTPHEADER][] = 'X-Override-Notification: ' . ActionMidtransConfig::$overrideNotifUrl;
        // Set payment idempotency-key to header
        if (ActionMidtransConfig::$paymentIdempotencyKey) ActionMidtransConfig::$curlOptions[CURLOPT_HTTPHEADER][] = 'Idempotency-Key: ' . ActionMidtransConfig::$paymentIdempotencyKey;

        // merging with ActionMidtransConfig::$curlOptions
        if (count(ActionMidtransConfig::$curlOptions)) {
            // We need to combine headers manually, because it's array and it will no be merged
            if (ActionMidtransConfig::$curlOptions[CURLOPT_HTTPHEADER]) {
                $mergedHeaders = array_merge($curl_options[CURLOPT_HTTPHEADER], ActionMidtransConfig::$curlOptions[CURLOPT_HTTPHEADER]);
            ...
            $curl_options = array_replace_recursive($curl_options, ActionMidtransConfig::$curlOptions, $headerOptions);
        }
